...
var toolbarlist = [{
	...
	iconCls:'icon-remove',
	handler:function(){
		batchOperation("ldbzsc.do", "ids", $("#tbInfosys"), "确定要删除吗?", true);
    }
},'-',{
	text:'修改',
	iconCls:'icon-remove',
...
$(function(){
	/**表格**/
	$('#tbInfosys').datagrid({
		url:'ldbzcx.do',//path+'/page/leadermember/data/leaderMemberList.json', 
		singleSelect:false,  
		striped:true,
		loadMsg:"正在加载数据，请稍候...",
		rownumbers:true,
...
function addInfosys()
{
	$('#personDialog').dialog({
		...
		closed : false,
		cache : false,
		href : "showDialog.do?url='ldbzxj.do'",
		modal : true,
		resizable : true,
		onClose : function() {
...
function delSingle(id){
	...
	$.messager.confirm("信息确认","你确定要删除选择的信息系统吗？", function(result) {
       	if(result == true) {
       		//batchOperation("ldbzsc.do", "id", "#tbInfosys", "确定要删除吗?", true);
       		var delUrl = "ldbzsc.do";
         	delUrl += "?id=" + ids.join(";");
         	$.ajax({
   				type: "POST",
