...
$(function() {
	$("#btnSearch").click(function() {
        var params = $('#tblShenhe').datagrid('options').queryParams; //先取得 datagrid 的查询参数  
       // alert($("#leibieSelection option:selected").val());
        ...
        $('#tblShenhe').datagrid('reload'); //设置好查询参数 reload 一下就可以了  
	});
});
...
function editInfosys(id)
{
	...
	$('#shenheDialog').dialog({
		...
		closed : false,
		cache : false,
		href : "showDialog.do?url='JigouShenjiXiangqing.do?id=" + id + "'",
		modal : true,
		minimizable : true,
		resizable : true,
		onClose : function() {
			$('#tblShenhe').datagrid('reload');
		}
	});
}
