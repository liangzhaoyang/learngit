...
public class test_all {
	...
	public static void main(String args[])throws IOException{
		...
		if(result_temp_i>original_community_alones){//通过result_temp_i>original_community_alones来判断原始网络是否经过GN算法分裂。
			...
			out.println("");
			out.println("");
			out.println("经程序分析，原始网络极有可能分裂为"+final_community_num+"个集团：");
			...
			for(i=original_community_alones; i<=Q_i; i++){//从数目为original_community_alones+1的原始网络下开始分裂，到分裂为Q_i+2个社团结束。
				...
				System.out.printf("\n将分裂为%d个集团，分裂后的情况为：",i+2);
				out.println("");
				out.printf("将分裂为%d个集团，分裂后的情况为：",i+2);
			...
			String path = "D:\\Graphviz2.26.3\\bin\\dot -Tjpg d:\\mypicture\\original.dot -o d:\\mypicture\\original.jpg";
			...
			for(int s=original_community_alones; s<=Q_i; s++){
				String path1 =  "D:\\Graphviz2.26.3\\bin\\dot -Tjpg d:\\mypicture\\devided"+Integer.toString(s);
			...
			out.println("");
			out.println("");
			out.printf("但是现实生活中真正的结果可能是上述分裂过程中情况中的其中一种，请用户斟酌。");  
		}
		else{
			System.out.println("\n\n原始网络不能再分裂了，它是最紧凑的。");
			out.println("");
