...
int ACtree_compute_shifts(pattern_tree *ptree)
{
	...
	//ACtree_init_GSshifts (ptree) ;          //alick gai
	// printf ("\n函数 ACtree_compute_shifts----------------------") ; // 测试使用 
	// printf ("\n----调用 ACtree_init_GSshifts-------------------\n") ; // 测试使用
	// ACtree_print_tree(ptree) ;  // 测试使用
	//ACtree_compute_GSshifts (ptree) ;       //alick gai
	// printf ("\n函数 ACtree_compute_shifts----------------------") ; // 测试使用
...
int match_resualt_printf(unsigned char *text, pattern_data *patterns, int npattern, matched_info_t matched_items[], int nmatched)
{
	...
	for (i = 0; i < nmatched; i++)
	{
		...
		printf("len is : %d\n", (patterns + matched_items[i].pattern_i)->len);
		a = (patterns + matched_items[i].pattern_i)->len;
		char *str = new char[a+1];
		for (int j = 0; j < a; j++)
		{
			str[j] = (patterns + matched_items[i].pattern_i)->data[j];
		};
...
//调用的函数
string WideCharToMultiChar(char* str, int leolen)

{
	...
	int len = WideCharToMultiByte(CP_ACP, 0, (LPCWCH)str, leolen, NULL, 0, NULL, NULL);
	//printf("len = %d\n", sizeof(str));
	char *buffer = new char[len + 1];

	WideCharToMultiByte(CP_ACP, 0, (LPCWCH)str, leolen, buffer, len, NULL, NULL);
